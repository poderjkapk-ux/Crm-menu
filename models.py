...
async def get_menu_data(session: AsyncSession = Depends(get_db_session)):
    categories_res = await session.execute(
        sa.select(Category)
        .where(Category.show_on_delivery_site == True) # Враховуємо видимість
        .order_by(Category.sort_order, Category.name)
    )
    products_res = await session.execute(
        sa.select(Product)
        .join(Category)
        .where(Product.is_active == True, Category.show_on_delivery_site == True) # Враховуємо видимість
    )
...
async def admin_categories(session: AsyncSession = Depends(get_db_session), username: str = Depends(check_credentials)):
    ...
    <tr>
        <td>{c.id}</td>
        <td>
            <form action="/admin/edit_category/{c.id}" method="post" class="inline-form" style="flex-wrap: wrap;">
                <input type="text" name="name" value="{html.escape(c.name)}" style="flex-grow: 1; min-width: 150px;">
                <input type="number" name="sort_order" value="{c.sort_order}" style="width: 80px;">
                <div class="checkbox-group" style="margin-left: 10px;">
                    <input type="checkbox" id="show_on_delivery_{c.id}" name="show_on_delivery_site" value="true" {'checked' if c.show_on_delivery_site else ''}>
                    <label for="show_on_delivery_{c.id}">🚚</label>
                </div>
                <div class="checkbox-group">
                    <input type="checkbox" id="show_in_restaurant_{c.id}" name="show_in_restaurant" value="true" {'checked' if c.show_in_restaurant else ''}>
                    <label for="show_in_restaurant_{c.id}">🍽️</label>
                </div>
                <button type="submit" style="margin-left: 10px;">💾</button>
            </form>
        </td>
        <td class='actions'>
            <a href='/admin/delete_category/{c.id}' onclick="return confirm('Ви впевнені?');" class='button-sm danger'>🗑️</a>
        </td>
    ...
    <div class="card">
        ...
        <form action="/admin/add_category" method="post">
            ...
            <div class="checkbox-group">
                <input type="checkbox" id="add_show_on_delivery" name="show_on_delivery_site" value="true" checked>
                <label for="add_show_on_delivery">Показувати на сайті доставки 🚚</label>
            </div>
            <div class="checkbox-group">
                <input type="checkbox" id="add_show_in_restaurant" name="show_in_restaurant" value="true" checked>
                <label for="add_show_in_restaurant">Показувати в меню ресторану 🍽️</label>
            </div>
    ...
    <div class="card">
        ...
        <table>
            <thead><tr><th>ID</th><th>Назва, сортування та видимість</th><th>Дії</th></tr></thead>
            <tbody>{rows or "<tr><td colspan='3'>Немає категорій</td></tr>"}</tbody>
        </table>
    </div>"""
...
async def edit_category(
    ...
    if category:
        ...
        category.show_on_delivery_site = show_on_delivery_site
        category.show_in_restaurant = show_in_restaurant
        await session.commit()
    return RedirectResponse(url="/admin/categories", status_code=303)
